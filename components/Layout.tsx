...
const Layout: React.FC<LayoutProps> = ({ children, activeState, onNavigate, userName, settings }) => {
  // Detection for Widget/Iframe mode via URL param
  ...
  return (
    <div className={`flex flex-col h-screen overflow-hidden ${highContrastStyle} ${settings.contrast === 'high' ? 'contrast-high' : ''}`}>
      {/* Top Header - Hidden in Widget mode to save space */}
      {!isWidget && (
      ...
      {/* Main Content Area */}
      <main className={`flex-1 overflow-y-auto ${isWidget ? 'pb-20 p-2' : 'pb-28 sm:pb-32 p-4 sm:p-10'}`}>
      ...
      {/* Navigation */}
      <nav className={`fixed bottom-0 left-0 right-0 border-t flex justify-around items-center z-30 ${isWidget ? 'p-1' : 'p-2 sm:p-3'} ${settings.contrast === 'high' ? 'bg-black border-yellow-400' : 'bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.05)]'}`}>
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => onNavigate(item.id)}
            className={`flex flex-col items-center justify-center py-2 px-2 sm:px-6 rounded-2xl sm:rounded-3xl transition-all min-w-[50px] sm:min-w-[80px] ${
              activeState === item.id 
          ...
          >
            <div className={`${isWidget ? 'scale-90' : 'scale-110 sm:scale-125'}`}>{item.icon}</div>
            <span className={`${isWidget ? 'text-[10px]' : 'text-xs sm:text-base'} mt-1 font-bold`}>{item.label}</span>
          </button>
        ))}
        
        {/* Quick Settings access for widget mode since header is hidden */}
        {isWidget && (
          <button 
            onClick={() => onNavigate(AppState.SETTINGS)}
            className={`flex flex-col items-center justify-center py-2 px-2 rounded-2xl ${activeState === AppState.SETTINGS ? 'text-sky-600 bg-sky-50' : 'text-slate-400'}`}
          >
            <div className="scale-90"><Icons.Settings /></div>
            <span className="text-[10px] mt-1 font-bold">Options</span>
          </button>
        )}
      </nav>
    </div>
